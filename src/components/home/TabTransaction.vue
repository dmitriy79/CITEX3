        ...
        <dl class="transaction-list">
          ...
          >
            <div class="transaction-list-title"  >
              ...
              ></i>
               <!-- <i 
                @click.stop="favorite(item.id,item.collect)"
                class="ico ico-star-fill"
                :class="{'ico-star':!item.collect}"
              ></i> -->
              <span v-if="selectedZoneIndex!==-1">{{item.name}}</span>
              <span v-if="selectedZoneIndex==-1&&!item.zoneCoinName">{{item.name}}/{{item.unit_coin_name}}</span>
               <span v-if="selectedZoneIndex==-1&&item.zoneCoinName">{{item.name}}/{{item.zoneCoinName}}</span>
            </div>
            <div>{{item.deal_price}}</div>
            <div
              class="red"
              :class="{green : item.increase}"
            >{{item.increase ? '+' : '-'}}{{item.increase_24H | decimal(2)}}%</div>
