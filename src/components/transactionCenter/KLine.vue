                ...
                this_vue.chart.onChartReady(function() {
					...
					chart.onIntervalChanged().subscribe(null, function (interval, obj) {
					     ...
					     buttons.forEach((item, index) => {
							...
							item.resolution === this_vue.chart.interval && updateSelectedIntervalButton(button);
                        	button.attr("data-resolution", item.resolution)
                                .attr("data-chart-type", item.chartType === undefined ? 1 : item.chartType)
                                .html("<span>"+ item.label +"</span>")
                                .on("click", function() {
									if (!this_vue.chart.changingInterval && !button.hasClass("selected")) {
...
//											});
						 ...
						 function updateSelectedIntervalButton(button) {
                        	this_vue.chart.selectedIntervalButton && this_vue.chart.selectedIntervalButton.removeClass("selected");
